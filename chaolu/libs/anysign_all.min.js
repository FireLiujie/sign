...
function anysignCommentCore(a, b, c, d) {
	...
	S.onclick = function () {
		// debugger
		p || (n != v.length ? custom_alert("\u8bf7\u5b8c\u6210\u7b7e\u540d", "\u786e\u8ba4") :
			(h(), signResCallback && signResCallback(c.cid, L[0].substring(22, L[0].length), L[0].substring(22, L[0].length), H, K, N, O), e(), testGenData()))
	};
	T.onclick = function () {
		if (!p)
			if (0 >= n) alert("\u5df2\u79fb\u9664\u5b8c\uff0c\u8bf7\u91cd\u65b0\u8fdb\u884c\u7b7e\u540d\uff01");
			else {
...
function bnToString(a) {
	...
	if (0 < e--)
		for (h < this.DB && 0 < (c = this[e] >> h) && (d = !0, f = int2char(c)); 0 <= e;) h < a ? (c = (this[e] & (1 << h) - 1) << a - h, c |= this[--e] >> (h += this.DB - a)) : (c = this[e] >> (h -= a) & b, 0 >= h && (h += this.DB, --e)), 0 < c && (d = !0), d && (f += int2char(c));
	return d ? f : "0"
}
...
var pubN = "",
	pubE = "10001",
...
function checkText(a) {
	...
	for (var b = ", . ' \" ; : ? /  \\  [ ] { } \x3c \x3e | ( ) \u00b7".split(" "), c = 0; c < b.length; c++)
		if (a == b[c]) return !0;
	return !1
}
...
function anysignWebImpl() {
	...
	this._isReadyToUpload = function () {
		...
		for (b in w)
			if (e = w[b], a = p[e.cid], (null == a || null == a.Data) && e.nessesary) return !1;
		return !0
	};
	...
	this._restoreBusinessSession = function (a, b) {
		...
		for (B in g.DataConfigs)
			for (F in m = g.DataConfigs[B].cid, g.DataObjs) q = g.DataObjs[F].Cid, m == q && (w[m] = g.DataConfigs[B], p[q] = g.DataObjs[F]);
		r = g.CachetArray;
		return d = c = !0
	};
...
function sign_confirm() {
	...
	apiInstance.showCommentDialog("30");
}
